...
class PlayerRPG {
	public:
		...
		int GetPlayerEquipWeapon();
		int GetPlayerEquipArmor();
		...
		void EquipPlayerWeapon(int itemWeaponIn);
		void EquipPlayerArmor(int itemArmorIn);
	...
	private:
		...
		int playerLevel;
		int playerDefense;
		int playerEquipWeapon;
		int playerEquipArmor;
		string playerName;
};
...
int PlayerRPG::GetPlayerEquipWeapon() {
	return playerEquipWeapon;
}

//Get currently equipped armor
int PlayerRPG::GetPlayerEquipArmor() {
...
void PlayerRPG::EquipPlayerWeapon(int itemWeaponIn) {
	playerEquipWeapon = itemWeaponIn;
	return;
}

//Set the current armor
void PlayerRPG::EquipPlayerArmor(int itemArmorIn) {
	...
	while (combatOngoing != false) {
		...
		if (playerIn == "attack") {
			...
			while (hitMonster == false) {
				...
				//Clears cin if user in fails
				else if (cin.fail()) {
					cout << "Invalid variable." << endl;
					cin.clear(); 
					cin.ignore(10000, '\n');
				}
				else if (playerInChoice <= CurrentMonsters.size() || playerInChoice >= 0) {
					...
					if (CurrentMonstersAlive.at(playerInChoice) == true) {
						...
						damageTaken = player.GetPlayerAttack() + player.GetPlayerEquipWeapon();
						CurrentMonstersHealth.at(playerInChoice) = CurrentMonstersHealth.at(playerInChoice) - player.GetPlayerAttack();
					}
					else {
						cout << "That monster is dead. Select another monster." << endl;
		...
		//If player inputs defend
			...
			for (i = 0; i < CurrentMonsters.size(); i++) {
				if (CurrentMonstersAlive.at(i) == true) {
					...
					damageTaken = player.GetPlayerDefense() - player.GetPlayerEquipArmor();
					damageTaken = CurrentMonstersAttack.at(i) - damageTaken;
...
void adventureGame(std::vector<MonstersRPG> monsterList, vector<int> rooms, int roomsWidthIn, int roomsHeightIn, int victoryRoom, vector<WorldMonsters> monstersPosition, vector<string> roomDesc, vector<Items> ItemsList, vector<Leveling> levels, int startingRoom) {
	...
	while (gameOn != false) {
		cout << endl << endl;
		cout << "Please input an action. Available actions are: move, attack, look, open, take, inv, use, drop, heal, quit." << endl;
		cin >> playerIn;
		...
		//If player input is take
			...
			while (playerItemSelTrue == false) {
				...
				//Clears cin if user in is not in
				else if (cin.fail()) {
					cout << "Invalid variable." << endl;
					cin.clear();
					cin.ignore(10000, '\n');
						...
						for (i = 0; i < ItemsList.size(); i++) {
							if (ItemsList.at(i).GetItemName() == player.playerInventory.at(playerItemSel)) {
								...
								if (ItemsList.at(i).GetItemType() == 2) {
									...
									//If weapon equip it
									playerItemSelTrue = true;
									player.EquipPlayerWeapon(ItemsList.at(i).GetItemSpecialVal());
								...
								if (ItemsList.at(i).GetItemType() == 3) {
									...
									//If armor equip it
									playerItemSelTrue = true;
									player.EquipPlayerArmor(ItemsList.at(i).GetItemSpecialVal());
